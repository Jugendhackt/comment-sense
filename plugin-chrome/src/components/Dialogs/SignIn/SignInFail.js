...
const SignInFail = observer((props) => {
    ...
    return (
        <Dialog open={dialogStore.openSignInFail} onClose={() => dialogStore.openSignInFail = false} fullScreen={true}>
            <DialogTitle>{langDe.signInErrTitle}</DialogTitle>
            <DialogContent>
                <DialogContentText>{langDe.signInErrText}</DialogContentText>
                <DialogActions>
                    <Button variant="contained" color="secondary"
                            onClick={() => dialogStore.openSignInFail = false}>{langDe.ok}</Button>
                </DialogActions>
            </DialogContent>
        </Dialog>
    );
